...
public class TicTacToeTester{
	...
	public static void main( String[] args ){
        ...
        do{
            ...
            ttt.displayBoard();
            while(g < 10 && !ttt.isWinner('X') && !ttt.isWinner('O')){
                boolean askAgain = false;
                do{
                    System.out.print("Please enter the move position for '"+p+"' \nROW: ");
                    r = in.nextInt();
                    System.out.print("COLUMN: ");
                    c = in.nextInt();
                    if (ttt.isValid(r, c) && ttt.playerAt(r, c) != 'X' && ttt.playerAt(r, c) != 'O'){
                        ttt.playMove(p, r, c);
                        askAgain = false;
                    } else {
                    askAgain = true;
                    ttt.displayBoard();
                    System.out.println("You cannot place your move at that position, try again.");
                    }
                } while (askAgain);    
                ttt.displayBoard();
                ...
                if (ttt.isWinner(p)){
                    System.out.print("'"+p+"' is a winner!\nWould you like to play again? (y/n): ");
                    String ans = in.next();
                    if (ans.equals("y")){
                        playAgain = true;
						p = ( p == 'X' ? 'O' : 'X' );
